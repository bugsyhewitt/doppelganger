...
def test_wheel_builds_cleanly(tmp_path):
    ...
    wheels = list(out.glob("doppelganger-0.5.0-*.whl"))
    sdists = list(out.glob("doppelganger-0.5.0.tar.gz"))
...
def test_wheel_installs_into_fresh_venv(tmp_path):
    ...
    assert version_out == "doppelganger 0.5.0", (
...
def test_wheel_version_importable_in_fresh_venv(tmp_path):
    """`import doppelganger; doppelganger.__version__` == '0.5.0' in the venv."""
    ...
    _run(
        [str(py), "-c", "import doppelganger; assert doppelganger.__version__ == '0.5.0'"],
    )
